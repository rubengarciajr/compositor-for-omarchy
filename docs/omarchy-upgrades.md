...
| Cursors | Paint tools show a circle the exact brush size (soft-edge ring, crosshair when tiny); `[` `]` size, `Shift+[` `]` hardness. Wand cursor for Magic Wand (with +/− badges), crosshair for marquee/lasso/crop/gradient/shape, I-beam for Type, pipette for Eyedropper, grab for Hand, zoom-in/out for Zoom, resize/rotate over handles. |
| Type tool | Type directly on the canvas (click to place, click text to edit, Esc / Ctrl+Enter), plus font family, weight, size, colour, alignment, leading, tracking in the header; text layers grow with their content. |
...
| Crop | Handles to resize the crop box after drawing it (Shift ratio, Alt from centre), drag inside to move, Enter applies, Esc cancels. |
| Tool modifiers | Space pans with any tool. Shift-click draws straight lines with Brush / Eraser / Clone / Blur; Alt samples a colour with Brush / Eraser; Alt-click Eyedropper sets the background; Alt-click Zoom zooms out; Shift / Alt while dragging Marquee or Shape constrain / draw from the centre; Ctrl-click Move auto-selects the layer under the pointer, Alt-drag duplicates, Shift constrains; Enter / Esc apply / cancel a crop. Every tool header lists its modifiers. |
| Retouch | Spot Healing (`J`) fills marks from their surroundings; Blur really blurs with a Strength slider; Brush / Eraser / Clone have a Spacing % control; Magic Wand has a Contiguous toggle; shape layers get Fill / Stroke / Width / Radius options. |
| Paint tools | Brush colour swatch in the header (it is the foreground colour). Brush, Eraser, Clone, Blur and Fill create a layer automatically when the active one cannot be painted (folder, adjustment, text, shape), and paint correctly on moved, scaled, rotated or pasted layers. |
...
| Files | `.comp` projects via native Save / Save As / Open dialogs (a `.comp` package folder or ZIP reloads live when another program changes it), Layer › Add Image… to bring image files in as layers (transparent padding trimmed on import; Layer › Trim Transparent Pixels at any time), `compositor photo.png` from the terminal, several documents open at once. |
