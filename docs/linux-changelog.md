...
### Changed — the painting tools are now 1:1 with Compositor for macOS
- **Brush engine rebuilt** on Compositor's model: a stroke accumulates coverage in a buffer and the layer is rebuilt as *base + colour through coverage at the stroke's opacity*. Opacity caps the whole stroke (overlapping passes never darken), soft tips use the Mac app's falloff and deposit spacing (1.5 % / 2.5 % of the diameter), hard tips stay antialiased without build-up, and a stroke can paint anywhere on the canvas: the layer bitmap grows to the paint and is trimmed back to it when the stroke ends. Painting on type or a shape rasterises it; folders and adjustment layers accept paint on their mask only; nothing creates a layer for you.
- **Brush header** matches the Mac app: Paint / Erase mode (`B` / `E`, Tab toggles; the Eraser is the Brush in Erase mode), Size field 1–2000 px, Hardness and Opacity sliders with percent fields, **Smoothing** (the brush trails the pointer on a string that long, in screen points), Color swatch, and the Black · Hide / White · Reveal picker when a **mask is the target** (click a mask thumbnail in the Layers panel; `X` swaps, `D` returns to black). Flow and Spacing are gone, as upstream.
- **Keys and gestures**: `[` `]` step the size by 20 %, `Shift+[` `]` step hardness in quarters, `1`–`9` / `0` set opacity (two digits within 0.6 s for an exact value; with the Move tool they set the layer's opacity), Esc cancels a stroke, Shift-click draws a line from where the last stroke ended, holding Shift while dragging locks the stroke to an axis, **right-drag resizes the tip** (Shift+right-drag adjusts hardness) with the plain right-click menu unchanged. Tips are parked per family: Brush + Spot Healing share one (40 px, hard), Clone Stamp and Smear keep their own (40 px, soft).
- **Smear** (`R`, formerly Blur) with Compositor's three modes: **Liquify** pushes pixels along the drag, **Blur** softens toward a Gaussian copy of the layer (going over again softens more), **Smudge** picks colour up and drags it along; Strength instead of Opacity; Tab cycles the mode.
- **Spot Healing** rebuilds the painted area from its surroundings with the Mac app's algorithm: Content-Aware (patch search over 24 directions and 5 distances), Proximity Match (nearby patches) and Create Texture (grain from the ring), blended by a seamless membrane. While you paint the area shows as a dark wash; the heal happens on release.
- **Clone Stamp**: Aligned toggle (the offset from the first stroke is kept), Sample This Layer / All Layers (Tab toggles), the source crosshair follows the pointer, Alt-click sets a new source, and a press without a source explains what to do.
- **Brush cursor** is the Mac app's ring (white over black at the true size, never smaller than a pixel) with a dashed core ring while adjusting hardness; hidden while Alt samples a colour or Space pans.
- The **Select** state (`A`) no longer has a rail button; the header reads "Select a tool".

### Changed — the selection tools are now 1:1 with Compositor for macOS
- **Headers** match the Mac app: Marquee has Shape (Rectangle / Ellipse, Tab switches), Lasso has Freehand / Polygonal (Tab), Magic Wand has Tolerance 0–255, Sample Size (Point, 3 by 3, 5 by 5 Average), Sample This Layer / All Layers and Contiguous. All three share a **Mode** picker (New / Add / Subtract; Shift adds and Option subtracts for one outline, and the picker highlights the held key), **Anti-alias** (hidden for rectangles, which snap to whole pixels), **Expand / Contract / Feather** buttons with their pixel fields, an "Empty selection" note and a Deselect button.
- **Marquee** draws whole-pixel rectangles; Shift already held at the press means Add and does not square, a fresh Shift mid-drag squares (and reshapes the box the moment Shift changes); Option is reserved for Subtract; a click that encloses nothing deselects in New mode.
- **Moving selections**: drag inside the selection in New mode to move the outline (whole pixels; what leaves the canvas comes back), arrow keys nudge it with a selection tool (Shift ×10); `Ctrl`-drag inside moves the **selected pixels** and `Ctrl+Alt`-drag copies them ("Move Pixels" / "Duplicate Pixels", one undo step); `Ctrl`-arrows move the pixels with any tool.
- **Magic Wand** matches per channel, alpha included (every channel within Tolerance of the reference), averages the sample area, reads the active layer alone or every visible layer, and re-selects afresh when you click inside the selection; nothing matched deselects in New mode.
- **Expand… / Contract…** stroke the outline with a round band (Expand stays on the canvas, Contract can empty the selection); **Feather…** stacks in quadrature up to 250 px and keeps the marching ants on the crisp outline while fills, strokes and clears use the soft coverage. Select › **Layer's Pixels** and **Mask's Black Areas** load selections from the active layer and its mask.
- **Lasso**: Delete removes the last polygon corner, the first corner closes within 8 screen pixels and carries an 8 × 8 handle, the rubber band follows the pointer, freehand keeps every point 0.25 px apart; undo names read "Lasso", "Polygonal Lasso", "Rectangular Marquee", "Elliptical Marquee", "Magic Wand".
- **Edit menu**: Cut (`Ctrl+X`), Fill with Foreground Color (`Alt+Backspace`), Fill with Background Color (`Ctrl+Backspace`), Clear Selection Pixels. **Delete** clears the selected pixels, deletes the mask when it is the target, and otherwise deletes the layer, as in the Mac app.
- **Marching ants** are the Mac app's: a white line under a black 4-on 4-off dash that steps a pixel every 120 ms, now chained into continuous loops so the dashes really march (they used to blink). Outline drafts are drawn black under white, as upstream.
- Gaussian Blur now blurs a copy instead of drawing the blur over the original (this also affects the Filter menu).

### Changed — Move / Transform is now 1:1 with Compositor for macOS
- **Transform header** (Compositor's inspector): Auto Select and Show Controls toggles (both remembered), X / Y / W / H fields, a link button that keeps the ratio (Shift inverts it while dragging), Scale %, angle, Sampling (Nearest / Smooth / High quality, saved in the `.comp` manifest), Flip H / Flip V, Cancel and Apply. Typing a value starts a **persistent edit**: Return or Apply commits it as one undo step, Esc restores the layer with no step. Plain drags still commit on release.
- **Transform Layer / Transform Selection** (`Ctrl+T`): with a selection the pixels float on a "Floating Selection" layer; transform them and Apply merges them back into their layer (growing it where they now extend past it) and moves the outline with them; Esc puts everything back exactly. `Ctrl+H` shows or hides the controls; hidden, any drag moves the layer.
- **Handles** are the Mac app's: 7 × 7 white squares with an accent stroke, a round **rotation knob** on a stem 28 px above the top edge (the "rotate outside a corner" band is gone), whole edges are draggable, hits are judged within 10 screen px, and dragging a handle past the opposite side **flips** the layer instead of stopping. Drags land on whole pixels and whole degrees; Shift snaps rotation to 15°; Option scales from the centre.
- **Auto Select**: a press need not land inside the layer; with Auto Select on (or `Ctrl` held) the topmost layer under the pointer is picked, preferring a layer stacked above a selected full-canvas background; `Ctrl+Shift`-click adds to the multi-selection. Option-drag duplicates at the first movement and moves the copy.
- **Snapping** while moving (Control bypasses it): the layer's edges and centre seek the canvas edges and centre, guides and other layers within 10 screen px, with accent lines drawn while they match.
- `Ctrl+]` / `Ctrl+[` move the layer up / down among its siblings; `1`–`0` set the layer's opacity; the Move cursor carries the four-way badge, Option shows the duplicate arrows, the rotation knob shows the rotate cursor.

### Changed — Crop, Gradient, Shape, Type, Eyedropper, Hand and Zoom are now 1:1 with Compositor for macOS
- **Crop** shows the whole canvas as its frame when the tool is picked (or the selection's bounds), Ratio has Free / Original / 1:1 / 4:3 / 3:4 / 16:9 / 9:16 (changing it keeps the left edge and width), the header reads the frame's size, and Cancel / Apply Crop. Whole edges are draggable (20 px bands), dragging inside moves the frame, a drag anywhere on the full-canvas frame starts a new one, Option draws symmetrically, the frame snaps to the canvas and layer bounds within 8 screen px (Control bypasses), and it may extend past the canvas: **cropping never resamples**, the canvas takes the frame's size and every layer keeps its pixels and just moves. The overlay is the Mac app's: darkened outside, a white frame, rule-of-thirds, 8 × 8 handles.
- **Gradient** fills the **active layer** (or its mask, white revealing) as a pending edit instead of adding a layer: drag to draw, drag either end to adjust (Shift snaps to 45°), Return or Apply commits as one step, Esc or the first `Ctrl+Z` discards, switching tool or layer applies. Header: Linear / Radial, the preset picker (Compositor's two ramps plus the Photoshop Basics kept from before), Reverse, an Opacity slider with a percent field (`1`–`0` type it), Cancel / Apply. The ramp extends past both ends; the overlay is the Mac app's line with 12 px end circles filled with the end colours and a dashed rim for Radial. Option is the eyedropper.
- **Shape**: Rectangle / Ellipse / Line (Shift-U or Tab steps), a Width slider and field for lines (1–5000, default 4), a Radius slider and field for rectangles (0–5000, default 0), and the Fill swatch is the foreground colour. Shift makes squares and circles or snaps lines to 45°, Option draws from the centre, the draft shows in the foreground colour, Esc cancels. Lines are stroked between their endpoints with round caps and keep them exact when scaled; layers are named "Rectangle 1", "Ellipse 2", "Line 3"; the selection is kept; undo names match.
- **Type**: 12 px padding around the glyphs as upstream, Size in pixels (1–2000, default 72), Tracking, **Leading in pixels with Auto** (120 % of the size), Cancel / Done while editing and an Edit Text button otherwise. A click puts the first baseline on the pointer; **drag to draw a text box** that wraps words. Option-arrows change tracking (left / right) and leading (up / down) while typing, by 10 with Shift. Layers are named after their content (40 characters). Fill with Foreground / Background on type recolours it and keeps it editable ("Fill Text"). New text starts in the foreground colour. Undo names: "New Text Layer", "Edit Text".
- **Eyedropper** always writes the foreground (no Option-to-background), shows Compositor's **sample ring** while the button is down (new colour on top, previous below; a Sample Ring toggle turns it off), and keeps sampling while you drag. Option is the eyedropper in Brush, Spot Healing and Gradient.
- **Zoom**: click doubles and Option-click halves about the click, dragging right or left zooms smoothly (doubling every 100 px), the header has a percent field (0.1–3200 %), `Ctrl+=` / `Ctrl+-` step through the Mac app's zoom levels, `Ctrl` or `Alt` + wheel zooms about the pointer, plain wheel pans, `Ctrl+0` fits with the Mac app's margin, and the View menu reads Fit Canvas / Actual Pixels / Zoom In / Zoom Out. **Hand** is titled Pan.
- **Colour well** restyled to the Mac app's 24 px swatches with the swap and default glyphs in their places.
...
- **Type box and glyphs line up.** Glyphs now sit centred in their line box (half the leading above, half below), so the box no longer looks bottom-heavy on a single line, and the on-canvas editor lays text out exactly the same way.
- **Crisp at 100 %.** The canvas is drawn at a whole device-pixel offset in the window (zooming and panning could leave it on a half pixel, which softened every edge, including type).
